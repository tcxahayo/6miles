      ...
      <div className="content" onClick={e => e.stopPropagation()}>
        ...
        <div className="message">
          ...
          <div className="mid">
            ...
            <input type="text" id="name" name="name" onChange={changeValue} value={form?.nickname} />
            <label htmlFor="password">密码:</label>
            <input type="password" placeholder="6-16位数字或字母" id="password" name="password" onChange={changeValue} value={form?.password} maxLength={16} />
            <i className="iconfont icno" style={show?{display:'inline-block'}:{display:'none'}}>&#xe638;</i>
            <label htmlFor="passwordAgain">确认密码:</label>
            <input type="password" id="passwordAgain" name="passwordAgain" onChange={changeValue} />
            <i className="iconfont icno2" style={right?{display:'inline-block'}:{display:'none'}}>&#xe638;</i>
            <i className="iconfont icno3" style={use?{display:'inline-block'}:{display:'none'}}>&#xe603;</i>
            ...
            <div className="send" onClick={sendPhone} style={send?{display:'inline-block'}:{display:'none'}}>获取</div>
  <div className="sended" style={send?{display:'none'}:{display:'inline-block'}}>重新获取<span>{time}</span></div>
          </div>
          <div className="submit" onClick={submit}>确定</div>
          <div className="login" onClick={login}>已有账号，登录</div>
        </div>
      </div>
    </div>
  )
